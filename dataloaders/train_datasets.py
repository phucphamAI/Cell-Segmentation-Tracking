...
class CellTrain(Dataset):
    ...
    def __len__(self):
        arr = []
        for key in self.imglistdic.keys():
            arr.append(len(self.imglistdic[key][0]))
        accum = np.add.accumulate(arr) - self.curr_length 
        return int(accum[-1])
    
    def decision_data(self,idx):
        arr = []
        for key in self.imglistdic.keys():
            arr.append(len(self.imglistdic[key][0]))
        accum = np.add.accumulate(arr)
        minus_index = list(accum - idx)
        index_data = min(i for i,n in enumerate(minus_index) if n>0)
        ...
        if idx < accum[0]:
            ...
            start = idx - accum[index_data - 1]
            
        if (minus_index[index_data] <= self.curr_length):
            start = start - self.curr_length
            

        return index_data, start
    
    
    def get_img_cell(self,path):
        return cv2.imread(path)
    def get_lst_img_cell(self,lst_path):
        return [cv2.imread(path) for path in lst_path]
    
    def get_label_cell(self,path):
        return np.array(Image.open(path))
    def get_lst_label_cell(self,lst_path):
        return [np.array(Image.open(path)) for path in lst_path]
    
    def sample_sequence(self, idx):
        
        index_data, i      = self.decision_data(idx)
        (img_path, label_path)  = self.imglistdic[index_data]
    
        curr_images        = self.get_lst_img_cell(img_path[i:i+self.curr_length])
        curr_labels        = self.get_lst_label_cell(label_path[i:i+self.curr_length])
        
        if (i == 0):
            prev_image      = self.get_img_cell(img_path[0])
            prev_label      = self.get_label_cell(label_path[0])
        else:
            prev_image      = self.get_img_cell(img_path[i - 1])
            prev_label      = self.get_label_cell(label_path[i - 1])
            
        
        if (i < self.ref_gaps):
            ref_image       = self.get_img_cell(img_path[0])
            ref_label       = self.get_label_cell(label_path[0])
        else:
            ref_image       = self.get_img_cell(img_path[i-self.ref_gaps])
            ref_label       = self.get_label_cell(label_path[i-self.ref_gaps])
        
        ref_objs = list(np.unique(ref_label))
        num_objs = list(np.sort(ref_objs))[-1]
...
class CELL_Train(CellTrain):
    def __init__(self,
        ...
        imglistdic = {}
        path_train_all = glob(f'{root}/*')
        path_train_gt = sorted(glob(f'{root}/*_gt'), key = lambda x: int(os.path.split(x)[1].split("_")[0]))
        path_train_img = sorted(list(set(path_train_all) - set(path_train_gt)), key = lambda x: int(os.path.split(x)[1].split("_")[0]))
        
        for i in range(len(path_train_img)):
            imgs = sorted(glob(os.path.join(path_train_img[i],"*")),key = lambda x: int(os.path.split(x)[1].split('t')[1].split('.')[0]))
            labels = sorted(glob(os.path.join(path_train_gt[i],"*")),key = lambda x: int(os.path.split(x)[1].split('man_seg')[1].split('.')[0]))
            imglistdic[i] = (imgs, labels)
        
        
        super(CELL_Train, self).__init__(
